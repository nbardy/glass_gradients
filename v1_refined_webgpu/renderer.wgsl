...
fn sample_budget(state: PixelState, frag_coord: vec2f) -> u32 {
  ...
  if (params.flags.x > 0.5 && count >= 12.0 && state.luma_stats.z >= 0.98) {
    return 0u;
  }
  ...
  let extra = u32(clamp(floor(extra_f), 0.0, f32(max_s - base_s)));
  return base_s + extra;
}

fn background_sample_budget(state: PixelState, rd: vec3f) -> u32 {
  if (params.flags.x > 0.5 && state.luma_stats.z >= 0.985 && state.mean_count.w >= 8.0) {
    return 0u;
  }
...
fn update_state(state: PixelState, sample_rgb: vec3f) -> PixelState {
  ...
  let sample_luma = luminance(sample_rgb);
  let old_count = next_state.mean_count.w;
...
fn tonemap_color(mean_rgb: vec3f) -> vec3f {
  ...
  let luma = luminance(col);
  col = mix(vec3f(luma), col, 1.08);
  col = aces_film(col);
...
fn display_color(mean_rgb: vec3f, confidence: f32) -> vec3f {
  if (params.flags.z > 0.5) {
    let cold = vec3f(0.10, 0.16, 0.24);
    let warm = vec3f(0.99, 0.71, 0.18);
...
fn main_compute(@builtin(global_invocation_id) gid: vec3u) {
  ...
  if (state.luma_stats.z >= 0.9) {
    atomicAdd(&frame_stats[1], 1u);
  }
  if (spp == 0u) {
    atomicAdd(&frame_stats[2], 1u);
  }
  if (spp > max(u32(params.sampling.x), 1u)) {
    atomicAdd(&frame_stats[3], 1u);
  }
  if (state.luma_stats.z < 0.5) {
    atomicAdd(&frame_stats[4], 1u);
  }
  if (state.luma_stats.x > 0.45 && state.luma_stats.z < 0.8) {
    atomicAdd(&frame_stats[5], 1u);
  }
  if (state.luma_stats.x < 0.08 && state.luma_stats.z < 0.8) {
    atomicAdd(&frame_stats[6], 1u);
  }
