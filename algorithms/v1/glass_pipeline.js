...
export async function v1GlassPipeline(device, canvas, shaderSource, config) {
    ...
    return {
        ...
        async render(timestamp) {
            ...
            if (runBackgroundPass) {
                ...
                backgroundPass.dispatchWorkgroups(Math.ceil(BACKGROUND_SIZE / WORKGROUP_SIZE), Math.ceil(BACKGROUND_SIZE / WORKGROUP_SIZE));
                backgroundPass.end();
            }
            // Glass compute pass
            ...
            glassPass.dispatchWorkgroups(Math.ceil(RENDER_SIZE / WORKGROUP_SIZE), Math.ceil(RENDER_SIZE / WORKGROUP_SIZE));
            glassPass.end();
            // Render pass
