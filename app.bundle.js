(() => {
  ...
  var GlassGenerator = class {
    ...
    constructor(device, shaderCode, config) {
      ...
      for (let i = 0; i < this.numDroplets; i++) {
        dropletsData[i * 4 + 0] = Math.random();
        dropletsData[i * 4 + 1] = Math.random();
        dropletsData[i * 4 + 2] = 1e-3;
        dropletsData[i * 4 + 3] = 0.02 + Math.random() * 0.05;
      }
  ...
  var BackgroundManager = class {
    ...
    async renderUnifiedSky(az, el, width, height, config) {
      ...
      state.controls.on("change", (key, value) => {
        ...
        if (key === "glassPatternType") {
          ...
          if (type === 0) {
            ...
            updates = { glassHeightAmpl: 8e-3, glassBump: 0.1, glassDropletProfile: 2.5, glassScale: 1, glassDistortion: 1, glassRoughness: 0.1 };
          } else if (type === 5) {
